...
export default function UserActivitiesScreen(props) {
  ...
  const getIndex = async(isInit) => {
    try {
      ...
      const indexList = await props.userActivitiesIndex('', isInit);
      if (indexList) {
        if (isInit) {
      ...
      > 
        <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1, paddingLeft: 10 }}>
          <Pressable style={{ flexDirection: 'row', alignItems: 'center', }} onPress={() => props.navigation?.pop()}>
            ...
            <Text size={20} bold={true} color={'#333'}>User Activities</Text>
          </Pressable>
        </View>
      </ImageBackground>
          ...
          {props.dataIndex?.data?.length > 0 ? 
            props.dataIndex?.data.map((item, index) =>
              ...
              <View style={{ flex: 1, flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                ...
                <Text bold size={16} color={'#555'}>Data Aktivitas Masih Kosong</Text>
                <Text size={14} color={'#888'} style={{ maxWidth: width - 120, textAlign: 'center' }}>Belum ada aktivitas yang kamu kerjakan di aplikasi ini.</Text>
              </View>
          }
        </ScrollView>
      </View>
    </View>
    {/* <Loader visible={onLoad || props.isLoading || props.isLoadingArea}/> */}
  </ImageBackground>
  );
}
